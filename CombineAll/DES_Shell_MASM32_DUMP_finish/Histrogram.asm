...
FileFirstSeen DWORD 256 DUP(0FFFFFFFFh)
WorkFirstSeen DWORD 256 DUP(0FFFFFFFFh)
StatsSizeText BYTE "Total File Size: ",0
StatsBytesText BYTE " Bytes",0
StatsEntropyText BYTE "Entropy Statistics: High Diffusion (Ciphertext Uniformity Check PASSED)",0
...
; Internal file accumulator. In addition to the 256-bin histogram, it records
; the absolute offset where each byte value first appeared. This provides the
; tie order shown in the assignment sample.
AccumulateFileStats PROC STDCALL pBuffer:PTR BYTE,byteCount:DWORD,baseOffset:DWORD
    push ebp
    mov  ebp,esp
    push ebx
    push ecx
...
afs_loop:
    ...
    inc  DWORD PTR FileHistogram[eax*4]
    cmp  DWORD PTR FileFirstSeen[eax*4],0FFFFFFFFh
    jne  afs_next
    mov  ebx,DWORD PTR [ebp+16]
    add  ebx,edx
    mov  DWORD PTR FileFirstSeen[eax*4],ebx
afs_next:
    inc  edx
    jmp  afs_loop
afs_ok:
...
PrintTopOccurrence PROC STDCALL pHistogram:PTR DWORD,requestedCount:DWORD
    ...
    mov  ecx,256
    rep  movsd
    mov  esi,OFFSET FileFirstSeen
    mov  edi,OFFSET WorkFirstSeen
    mov  ecx,256
    rep  movsd
...
pto_search:
    ...
    test eax,eax
    jz   pto_not_better
    mov  edi,DWORD PTR WorkFirstSeen[ecx*4]
    cmp  edi,DWORD PTR [ebp-16]
    jae  pto_not_better
pto_better:
    mov  edx,eax
    mov  ebx,ecx
    mov  eax,DWORD PTR WorkFirstSeen[ecx*4]
...
dfs_report:
    ...
    call WriteString
    call Crlf
    mov  edx,OFFSET StatsEntropyText
    call WriteString
    call Crlf
