...
- `displayHexDump.asm` prints 16 bytes per line, an eight-digit offset, aligned
  hex columns, and printable ASCII with `.` outside `20h..7Eh`. The shell DUMP
  command is configured to preview only the first 16 bytes (offset 00000000) to
  match the requested reference screenshot; it does not truncate the file.
